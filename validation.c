...
int is_macro(char *line, int *error){
	int c = 0; 
	char *p = line;
	char macro[MAX_MACRO_LEN];
	char key[MAX_MACRO_LEN];
	char value[MAX_MACRO_LEN];
	...
	/* enters to symbol table */
	if(search_sym(key) == NULL)
	{
		to_symbol(key,atoi(value), FALSE, FALSE, TRUE);
		return TRUE;
	}				
	else
	{
		/* macro cant be declared twice */
		*error = MACRO_ERROR;
		return NA;
	}
}
...
int check_macro(char *line, int *error){
	int c = 0; 
	char *p = line;
	char macro[MAX_MACRO_LEN];
